...
const { ER_ADMIN_WTH_EMAIL_ID_ALREADY_EXISTS, ER_INVALID_POLLINGBOOTH_ID, ER_POLLINGBOOTH_ALREADY_LINKED } = require("../common/errors");
...
const createAdmin = async (admin) => {
	...
	if (existingPollingBooth) {
		const adminAttachedToPollingBooth = await adminRepository.getAdminByPollingBoothId(admin.pollingBoothId);
		if (!adminAttachedToPollingBooth) {
			const existingAdmin = await adminRepository.getAdminByEmailId(admin.email);
			if (!existingAdmin) {
				const result = await adminRepository.createAdmin({ ...admin, role: constants.roles.PBO });
				const structuredResult = await getClientPresentableResult(result);
				return structuredResult;
			} else {
				throw ER_ADMIN_WTH_EMAIL_ID_ALREADY_EXISTS;
			}
		} else {
			throw ER_POLLINGBOOTH_ALREADY_LINKED;
		}
	} else {
